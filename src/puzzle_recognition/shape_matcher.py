...
_SLOT_MASK_CACHE: dict[tuple[str, str], np.ndarray] = {}


def load_slot_mask(
    board_id: str,
    slot: dict[str, Any],
    board_root: Path = BOARDS_DIR,
    use_cache: bool = True,
) -> np.ndarray:
    import cv2
    ...
    if use_cache:
        _SLOT_MASK_CACHE[cache_key] = binary
...
def match_piece_to_slot_iou(
    ...
    def place_mask_local(mask: np.ndarray, canvas_shape: tuple[int, int], target_cx: float, target_cy: float) -> np.ndarray:
        h, w = mask.shape[:2]
        moments = cv2.moments((mask > 0).astype(np.uint8))
        if moments["m00"]:
    ...
    use_soft_iou = bool(get("use_soft_iou", True))
    soft_mask_ksize = int(get("soft_mask_ksize", 5))
