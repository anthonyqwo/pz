...
# Let's run matching for ALL 4 pieces using both Original + Cache and Optimized + Cache
print("=== TESTING PRUNED CANDIDATES (max_candidates=8, coarse_step=15) ===")
...
t_opt_end = time.time()
print(f"Optimized with Cached Masks: {t_opt_end - t_opt_start:.4f}s")
print(opt_results)

speedup = (t_orig_end - t_orig_start) / (t_opt_end - t_opt_start)
print(f"Speedup of Optimized over Original: {speedup:.2f}x!")
