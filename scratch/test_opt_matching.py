...
from puzzle_recognition.shape_matcher import (
    crop_mask, rotate_mask, mask_centroid, mirror_variants,
...
piece = pieces[0]
slot = board_config["slots"][427]  # slot_428 or slot_427 which is highly matching
...
def optimized_match(piece_mask, slot_mask, cfg):
    ...
    # Calculate centroid of slot_crop once
    ...
    # Pre-align piece_crop onto a square canvas so that rotating it around its center
    # keeps its centroid perfectly centered at local_center.
    ...
    # We create a padded square piece canvas where piece centroid is exactly at the center.
    ...
    def evaluate_fast(variant_centered_canvas, angle, mirror_mode, dx_list, dy_list, is_refining=False):
        nonlocal best
        # Rotate the pre-centered canvas around its center (pcx, pcy)
        ...
        # Place rotated mask on local_shape, centered at local_center.
        # Since rotated's centroid is mathematically guaranteed to be at (pcx, pcy),
        # we can place it directly without calculating moments!
    ...
    # Stage 1: Coarse search. Evaluate all coarse angles with dx=0, dy=0.
    coarse_candidates = []
    ...
    for _, best_coarse_angle, mirror_mode, v_canvas in top_candidates:
        ...
        for angle in refine_angles:
            evaluate_fast(v_canvas, angle % 360, mirror_mode, translations, translations, is_refining=True)

    return best


# Let's benchmark both!
print("Benchmarking ORIGINAL match_piece_to_slot_iou...")
t0 = time.time()
orig_res = original_match(piece_mask_full, slot_mask, cfg)
t1 = time.time()
orig_time = t1 - t0
print(f"Original: Time={orig_time:.4f}s, IoU={orig_res['iou']:.4f}, Rot={orig_res['rotation']}, dx={orig_res['dx']}, dy={orig_res['dy']}")

print("\nBenchmarking OPTIMIZED match_piece_to_slot_iou...")
t0 = time.time()
opt_res = optimized_match(piece_mask_full, slot_mask, cfg)
t1 = time.time()
opt_time = t1 - t0
print(f"Optimized: Time={opt_time:.4f}s, IoU={opt_res['iou']:.4f}, Rot={opt_res['rotation']}, dx={opt_res['dx']}, dy={opt_res['dy']}")

print(f"\nSpeedup: {orig_time / opt_time:.2f}x!")
print("Are results virtually identical?", abs(orig_res['iou'] - opt_res['iou']) < 0.02 and orig_res['rotation'] == opt_res['rotation'])
